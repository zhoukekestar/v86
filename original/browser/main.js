...
(function () {
  ...
  function AsyncXHRBuffer(filename, block_size, size) {
    ...
    this.get_buffer = function (fn) {
      ...
      if (size > 32 * 1024 * 1024) {
        dbg_log(
          'Warning: Downloading all parts of a huge file. Will probably ' +
  ...
  window.onload = function () {
    ...
    if (DEBUG) {
      $('start_test').onclick = function () {
        settings.floppy_disk = new AsyncXHRBuffer(
          'images/fd/freedos.part%d.img',
          512,
          720 * 1024
        );
        init(settings);
  ...
  function init(settings) {
    ...
    document.getElementsByClassName('phone_keyboard')[0].style.display =
      'block';
